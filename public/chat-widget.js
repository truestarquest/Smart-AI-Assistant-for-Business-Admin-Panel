...
const GIBBERISH_WORD_RE = /[^\s]{35,}/;             // any 35+ character run with no whitespace — catches keyboard-mash text that varies character-by-character (so CHAR_FLOOD_RE's repeat check doesn't trigger)
...
function isSpamInput(text) {
  ...
  if (GIBBERISH_WORD_RE.test(text)) return true;
